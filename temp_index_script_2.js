                ...
                if (!selectedShopNumber) {
                    ...
                    headerCard.innerHTML = `
                        <span class="flex items-center gap-1.5 text-amber-400 font-extrabold">
                            ...
                            ผลการค้นหา (${displayStores.length} ร้าน)
                        </span>
                    `;
        ...
        async function navigateToGate() {
            ...
            if (!sMatch && svg) {
                ...
                pathElement.setAttribute('fill', 'none');
                pathElement.setAttribute('stroke', '#22c55e');
        ...
        async function selectPreorderStore(storeId, name) {
            try {
                ...
                if (storeLabel) storeLabel.textContent = 'ðŸ“ ' + name;
                preorderCart = {};
                await loadProducts();
                ...
                return `<div class="bg-white rounded-2xl p-3 shadow-sm border border-gray-100 flex flex-col h-full ${outOfStock ? 'opacity-50' : ''}">
                    ...
                    <div class="flex-1 flex flex-col">
                        ...
                        <div class="mt-auto">
                            ...
                            ${outOfStock ? `<button disabled class="w-full bg-gray-100 text-gray-400 py-2 rounded-xl text-xs font-bold">สินค้าหมด</button>` : (cartQty === 0
        ...
        async function trackOrder() {
            ...
            try {
                ...
                const storesMap = { 'DE40': 'PANPURI Concourse D East', 'DE12': 'PANPURI Concourse D East 2', 'DW41': 'PANPURI Concourse D West' };
                const items = Array.isArray(o.items) ? o.items : [];
                const staffNote = o.staff_note ? `<div class="mt-3 bg-orange-50 border border-orange-200 rounded-xl p-3 text-sm text-orange-700 font-semibold">ðŸ’¬ หมายเหตุจากร้านค้า: ${o.staff_note}</div>` : '';
                ...
                resultEl.innerHTML = `
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                        ...
                        <div class="border-t border-gray-100 pt-3 mt-3 space-y-1.5">
                            ${items.map(i => `<div class="flex justify-between text-xs"><span class="font-semibold text-gray-700">${i.name} Ã— ${i.qty}</span><span class="font-bold text-gray-900">฿${(parseFloat(i.price)*parseInt(i.qty)).toLocaleString()}</span></div>`).join('')}
                        </div>
                        ...
                        <p class="text-[10px] text-gray-400 mt-3 mb-2">ลูกค้า: ${o.customer_name} â€¢ ไฟลท์: ${o.flight_number}</p>
                        <div class="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                            <div>
                                <div class="text-[10px] text-gray-400">รับสินค้าที่ร้าน</div>
                            ...
                            <button onclick="closeAIModal(); showPage('map-view'); document.getElementById('map-search-input').value = '${o.store_id}'; setTimeout(()=>searchMapLive(),100);" class="bg-blue-50 border border-blue-100 text-blue-600 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 hover:bg-blue-100 transition-all active:scale-95 shadow-sm">
                                <span class="material-symbols-outlined text-xs">near_me</span> นำทาง
                            </button>
                        </div>
                        ${o.status === 'pending' ? `
                        <div class="mt-4 pt-3 border-t border-gray-100 text-center">
                            <button onclick="cancelCustomerOrder('${o.order_number}')" class="text-[10px] font-bold text-gray-400 hover:text-red-600 border border-gray-200 hover:border-red-200 hover:bg-red-50 px-3 py-1.5 rounded-full transition-all active:scale-95 inline-block">ยกเลิกรายการจองนี้</button>
                        </div>
                        ` : ''}
                    </div>`;
            } catch(e) {
