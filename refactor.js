...
// 1. Add DB import and remove CSV
s = s.replace(/const csv = require\('csv-parser'\);/g, 'const db = require(\'./db\');');

// 2. Replace readCsv function
const readCsvRegex = /function readCsv\(filePath\).*?\}\n\}/s;
const newReadCsv = `async function readCsv(filePath) {
  const tableMap = {
...
s = s = s.replace(readCsvRegex, () => newReadCsv);

// 3. Replace saveStoresToCsvSync
const saveStoresRegex = /function saveStoresToCsvSync.*?\}\n\}/s;
const newSaveStores = `async function saveStoresToCsvSync(stores) {
  try {
...
s = s = s.replace(saveStoresRegex, () => newSaveStores);

// 4. Replace saveNodesToCsvSync
const saveNodesRegex = /function saveNodesToCsvSync.*?\}\n\}/s;
const newSaveNodes = `async function saveNodesToCsvSync(nodes) {
  try {
...
s = s = s.replace(saveNodesRegex, () => newSaveNodes);

// 5. Replace writeCsvGeneric
const writeCsvRegex = /function writeCsvGeneric.*?\}\n\}/s;
const newWriteCsv = `async function writeCsvGeneric(filePath, rows, headers) {
  const tableMap = {
    [WALK_TIME_CSV]: 'walk_time_matrix',
    [STORE_CSV]: 'store_matrix',
    [MAP_NODES_CSV]: 'airport_map_nodes',
  ...
  try {
    ...
    for (let row of rows) {
      let vals = headers.map(h => {
        // Fix for json objects which were saved as strings in CSV
        if (h === 'items_json' && typeof row[h] === 'string') {
          return row[h]; // raw string is fine for jsonb
        } else if (h === 'items_json' && typeof row[h] === 'object') {
...
s = s = s.replace(writeCsvRegex, () => newWriteCsv);

// Add 'await' to all calls to saveStoresToCsvSync, saveNodesToCsvSync, and writeCsvGeneric
