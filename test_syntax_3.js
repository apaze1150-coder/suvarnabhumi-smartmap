        ...
        function renderCategoriesUI() {
            ...
            if (sidebarContainer) {
                let sidebarHTML = `
                <!-- ALL ITEMS -->
                <div class="group px-4 py-3 cursor-pointer rounded-lg transition-all sidebar-cat-btn bg-secondary-container text-navy-luxury shadow-sm" data-cat="all" onclick="filterProducts('all')">
                ...
                Object.values(catMap).forEach(catData => {
                    if (catData.subs.size > 0) {
                        sidebarHTML += `
                        <details class="group">
                            <summary class="flex items-center justify-between text-navy-luxury hover:bg-surface-container rounded-lg px-4 py-3 cursor-pointer list-none transition-all sidebar-cat-btn" data-cat="${catData.originalName}" onclick="filterProducts('${catData.originalName}')">
                        ...
                        sidebarHTML += `
                        <div class="group px-4 py-3 cursor-pointer rounded-lg transition-all sidebar-cat-btn" data-cat="${catData.originalName}" onclick="filterProducts('${catData.originalName}')">
                            <span class="font-label-md text-label-md tracking-wider uppercase text-navy-luxury hover:text-[#c4a46d] transition-colors">${catData.originalName}</span>
                        </div>
                        `;
                    }
                });
                ...
                return `<div class="group bg-surface-container-lowest border border-outline-variant/30 rounded-xl overflow-hidden cursor-pointer haptic-active shadow-sm transition-all ${outOfStock ? 'opacity-60 grayscale-[0.2]' : 'hover:shadow-md'}" onclick="openBoutiqueDrawer('${p.product_id}')">
                    ...
                    <div class="p-md flex flex-col h-40">
                        ...
                        <div class="flex items-center justify-between mt-auto">
                            ...
                            ${outOfStock ? `<button disabled class="bg-surface-variant text-on-surface-variant w-10 h-10 rounded-full flex items-center justify-center opacity-50 cursor-not-allowed"><span class="material-symbols-outlined">add</span></button>` : (cartQty === 0 
                                ...
                                : `<div class="flex items-center bg-secondary-container rounded-full px-1 py-1" onclick="event.stopPropagation()">
                                    ...
                                    <button onclick="changeBoutiqueCartQty('${p.product_id}', 1)" ${cartQty >= stockQty ? 'disabled class="w-8 h-8 rounded-full flex items-center justify-center opacity-50"' : 'class="w-8 h-8 rounded-full flex items-center justify-center hover:bg-black/10 transition-colors"'}><span class="material-symbols-outlined text-sm">add</span></button>
                                   </div>`
                            )}
                        </div>
                    </div>
