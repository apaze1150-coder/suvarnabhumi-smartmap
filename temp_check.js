            ...
            const boutiqueHTML = filtered.map(p => {
                ...
                return `<div class="group bg-surface-container-lowest border border-outline-variant/30 rounded-xl overflow-hidden cursor-pointer haptic-active shadow-sm transition-all ${outOfStock ? 'opacity-60 grayscale-[0.2]' : 'hover:shadow-md'}" onclick="openBoutiqueDrawer('${p.product_id}')">
                    ...
                    <div class="p-md flex flex-col h-40">
                        <p class="text-label-sm text-secondary uppercase tracking-widest mb-1 truncate">${(p.sub_category || p['Sub-Category']) && (p.sub_category || p['Sub-Category']) !== '-' ? (p.sub_category || p['Sub-Category']) : 'Information not available'}</p>
                        <h3 class="font-headline-md text-headline-md text-primary mb-1 line-clamp-2">${(p.scent || p.Scent) && (p.scent || p.Scent) !== '-' ? (p.scent || p.Scent) : 'Information not available'}</h3>
                        <div class="flex items-center justify-between mt-auto">
                            <div>
                                <span class="font-headline-md text-headline-md text-primary block">฿${parseFloat(p.price).toLocaleString()}</span>
